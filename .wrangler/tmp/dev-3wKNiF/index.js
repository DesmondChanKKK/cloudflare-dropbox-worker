...
// .wrangler/tmp/bundle-aKKJNN/checked-fetch.js
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    console.log(`[Debug] Received request for filename: "${filename}", folder: "${folder}", type: "${requestType}"`);
    if (!filename) {
      console.error("[Debug] Error: No filename provided");
      return new Response('Please provide a "filename" query parameter.', { status: 400 });
    }
    let dropboxPath = "";
    if (folder) {
    ...
    if (env.DROPBOX_REFRESH_TOKEN && env.DROPBOX_APP_KEY && env.DROPBOX_APP_SECRET) {
      ...
      try {
        ...
        if (!tokenResponse.ok) {
          ...
          return new Response(`Dropbox Auth Error: ${tokenResponse.status} ${errorText}`, { status: 500 });
        }
        ...
        return new Response(`Auth Flow Error: ${error.message}`, { status: 500 });
      }
    } else {
    ...
    if (!accessToken) {
      ...
      return new Response("Configuration Error: No valid Dropbox Token found. Please set DROPBOX_ACCESS_TOKEN or (DROPBOX_REFRESH_TOKEN + APP_KEY + APP_SECRET).", { status: 500 });
    }
    try {
      console.log(`[Debug] Downloading file from Dropbox: ${dropboxPath}`);
      ...
      if (!dropboxResponse.ok) {
        ...
        return new Response(`Dropbox Download Error: ${dropboxResponse.status} ${errorText}`, { status: dropboxResponse.status });
      }
      console.log("[Debug] File downloaded, parsing Excel...");
      const arrayBuffer = await dropboxResponse.arrayBuffer();
      const workbook = readSync(new Uint8Array(arrayBuffer), { type: "array" });
      const firstSheetName = workbook.SheetNames[0];
      console.log(`[Debug] Parsing sheet: "${firstSheetName}"`);
      ...
      if (env.EXTRACTION_CONFIG) {
        try {
          ...
          console.log(`[Debug] Loaded extraction configs. Available types: ${Object.keys(allConfigs).join(", ")}`);
        } catch (e) {
      ...
      if (requestType === "custom") {
        ...
        try {
          ...
          console.log(`[Debug] Raw Body Text: "${rawBodyText}"`);
          let bodyConfig = null;
          if (rawBodyText) {
            try {
          ...
          console.log(`[Debug] Parsed Body Content: ${JSON.stringify(bodyConfig)}`);
          ...
          if (bodyConfig && Array.isArray(bodyConfig)) {
            ...
            console.log(`[Debug] Using CUSTOM configuration from BODY with ${extractionRules.length} rules.`);
          }
        } catch (e) {
          console.log(`[Debug] Failed to parse request body for custom config: ${e.message}`);
        }
        if (extractionRules.length === 0) {
          if (customConfigStr) {
            try {
              extractionRules = JSON.parse(customConfigStr);
              console.log(`[Debug] Using CUSTOM configuration from URL parameter with ${extractionRules.length} rules.`);
            } catch (e) {
              console.error(`[Debug] Failed to parse custom 'config' parameter: ${e.message}`);
              return new Response('Invalid JSON in "config" parameter.', { status: 400 });
            }
          } else {
            console.error('[Debug] Type is "custom" but no configuration provided.');
            return new Response('Type is "custom" but missing configuration in Body or "config" query parameter.', { status: 400 });
          }
        }
      } else {
        ...
        if (!extractionRules) {
          ...
          console.log(`[Debug] Using configuration for type: "${requestType}" with ${extractionRules.length} rules.`);
        }
      }
      const parseCurrency = /* @__PURE__ */ __name((value) => {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        for (const rule of extractionRules) {
          ...
          if (match) {
            ...
            console.log(`[Debug] Found ${rule.key}: ${parsedVal}`);
          }
        }
      }
...
// .wrangler/tmp/bundle-aKKJNN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aKKJNN/middleware-loader.entry.ts
