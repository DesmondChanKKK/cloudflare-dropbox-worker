...
// .wrangler/tmp/bundle-6LjrVH/checked-fetch.js
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    const isDebug = env.IS_DEBUG !== void 0 ? env.IS_DEBUG === "true" : DEFAULT_IS_DEBUG;
    const logger = {
    ...
    try {
      const downloadFile = /* @__PURE__ */ __name(async (path) => {
      ...
      const searchFileRecursively = /* @__PURE__ */ __name(async (searchFolder, searchFilename) => {
        ...
        try {
          while (hasMore) {
            let url2 = "https://api.dropboxapi.com/2/files/list_folder";
            let body = {
            ...
            if (cursor) {
              url2 = "https://api.dropboxapi.com/2/files/list_folder/continue";
              body = { cursor };
            }
            ...
            const match = data.entries.find(
              (entry) => entry[".tag"] === "file" && entry.name.toLowerCase() === lowerSearchFilename
            );
            if (match) {
      ...
      const extractData = /* @__PURE__ */ __name((rows2, rules) => {
        const extracted = {};
...
// .wrangler/tmp/bundle-6LjrVH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6LjrVH/middleware-loader.entry.ts
